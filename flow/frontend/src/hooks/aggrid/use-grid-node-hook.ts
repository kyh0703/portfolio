import {
  getNodePropertyClipboard,
  useAddNodePropertyClipboard,
...
export default function useGridNodeHook<T>(
  ...
  const { mutateAsync: addPropertyClipboardMutate } =
    useAddNodePropertyClipboard<T>()
  ...
  const cut = useCallback(async () => {
    const selectedData = removeSelectedRows()?.remove.map((row) => row.data!)
    if (selectedData) {
      await addPropertyClipboardMutate({
        data: {
          ip: localIp,
          type: 'cut',
  ...
  }, [removeSelectedRows, addPropertyClipboardMutate, localIp])

  const copy = useCallback(async () => {
    const selectedData = getSelectedRows()?.map((row) => row.data!)
    if (selectedData) {
      await addPropertyClipboardMutate({
        data: {
          ip: localIp,
  ...
  }, [getSelectedRows, addPropertyClipboardMutate, localIp])

  const paste = useCallback(async () => {
    const clipboard = await getNodePropertyClipboard<T>(localIp)
    if (!clipboard) {
      return
    }
    clipboard.property?.map((row) => addRow(row))
  }, [addRow, localIp])
        ...
        switch (event.key) {
          case 'a':
            if (event.ctrlKey || event.metaKey) {
              getRows().length === getSelectedRows()?.length
                ? deselectAll()
                : selectAll()
            }
            break
