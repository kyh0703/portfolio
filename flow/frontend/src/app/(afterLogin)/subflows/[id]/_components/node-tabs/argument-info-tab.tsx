...
export default function ArgumentInfoTab(props: NodePropertyTabProps) {
  ...
  const handleModalSubmit = (payload: {
    ...
    if (mode === 'update' && type === 'in') {
      updateInRowByRowIndex(data, getInSelectedRow()?.rowIndex)
      return
    }
    if (mode === 'update' && type === 'out') {
      updateOutRowByRowIndex(data, getOutSelectedRow()?.rowIndex)
      return
    }
  }
  ...
  const handleInParamButtonClick = async (type: 'get' | 'view') => {
    ...
    const inParams = subFlow.args.in.param.map(({ name }) => name)
