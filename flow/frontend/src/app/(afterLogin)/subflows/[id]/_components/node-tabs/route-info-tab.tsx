...
import { RouteInfo, RouteList } from '@/models/property/common'
...
import RouteInfoModal from '../node-modals/route-info-modal'
...
export default function RouteInfoTab(props: NodePropertyTabProps) {
  ...
  const {
    ...
    paste,
    remove,
  } = useGridNodeHook<RouteList>(gridRef, {
  ...
  const handleRowDoubleClicked = (event: RowDoubleClickedEvent<RouteList>) => {
    openModal('property-modal', { mode: 'update', data: event.data })
  }

  const handleModalSubmit = (mode: 'create' | 'update', data: RouteList) => {
    if (mode === 'create') {
      addRow(data)
    } else {
  ...
  return (
    <div className="flex h-full w-full flex-col">
      ...
      <Modal id="property-modal" title="Route">
        <RouteInfoModal tabProps={props} onSubmit={handleModalSubmit} />
      </Modal>
