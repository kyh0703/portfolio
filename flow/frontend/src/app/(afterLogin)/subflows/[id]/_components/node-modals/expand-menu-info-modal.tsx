...
export default function ExpandMenuInfoModal({
  ...
  disable: boolean
  onSubmit?: (mode: 'create' | 'update', data: BranchList) => void
}) {
  const id = useModalId()
  ...
  const menuName = useMemo(() => {
    const menu = menuList.find((menu) => menu.property.id === watchMenuId)
    return menu ? menu.property.name : ''
  }, [menuList, watchMenuId])
  ...
  const handleMoveMenuClick = async () => {
    const selectedMenu = menuList.find(
      (menu) => menu.property.id === watchMenuId,
    )
    if (!selectedMenu) {
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <FormAutocomplete
              ...
              name="menuId"
              options={options}
              selectOptions={menuList.map((menu) => menu.property.id)}
              onValueChange={onValueChange}
            />
