...
export default function ChatGrid({ tabValue }: { tabValue: number }) {
  const { control, getValues } = useFormContext<EntityList>()
  ...
  useEffect(() => {
    const codeData = getValues(`chatInfo.output.category.${tabValue}.codeData`)
    setRows(codeData)
    ...
    <>
      ...
      <div className="flex-1">
        <FormGrid
          ref={gridRef}
          control={control}
          name={`chatInfo.output.category.${tabValue}.codeData`}
