...
import { useQueryDefine, useUpdateDefine } from '@/services/define'
...
export default function IntentDetailForm({
  scope,
  databaseId,
}: {
  scope: DefineScope
  ...
  const [rowData, setRowData] = useState<EntityList[]>([])
  ...
  const { data } = useSuspenseQuery(useQueryDefine<DefineIntent>(databaseId))
  const { mutate: addSubFlowMutate } = useAddSubFlow()
  const updateMutation = useUpdateDefine<DefineIntent>()
  ...
  const subFlowsOptions = useMemo(() => {
    return subFlowList.map((subCall) => subCall.name)
  }, [subFlowList])

  useEffect(() => {
    setRowData(data.property.entityList ?? [])
  }, [data.property.entityList])
  ...
  const onSubmit = (data: DefineIntent) => {
    ...
    updateMutation.mutate(
      ...
      {
        onSuccess: () => router.push(`/defines/${scope}/intent`),
      },
    )
  }
  ...
  const openSubFlow = () => {
    if (!subFlowsOptions.find((subCall) => subCall === watchSubCall)) {
      toast.warn('SubCall을 찾을 수 없습니다.')
      return
    }

    const subFlowId = subFlowList.find(
      (subCall) => subCall.name === watchSubCall,
    )!.id
  ...
  const createSubCall = () => {
    ...
    if (!subFlowsOptions.find((subFlow) => subFlow === watchSubCall)) {
      addSubFlowMutate(
        {
          name: watchSubCall!,
          version: '1.0.0',
