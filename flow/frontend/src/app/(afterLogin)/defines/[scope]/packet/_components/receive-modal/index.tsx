...
const getRepeatTypeOptions = (
  ...
  // NOTE: RepeatR1 => Repeat1
  const filterRepeat = Array.from(
    new Set(
      repeat
        ?.filter((row) => row.type.includes('RepeatR'))
        .map((row) => row.type.replace('RepeatR', 'Repeat')),
    ),
  )

  const notAssign = filterRepeat.filter(
