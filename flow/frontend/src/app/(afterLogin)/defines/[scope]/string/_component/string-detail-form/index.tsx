...
export default function StringDetailForm({
  scope,
  databaseId,
}: {
  scope: DefineScope
  ...
  const updateMutation = useUpdateDefine<DefineString>({
    onSuccess: () =>
  ...
  const { data, stringList } = useSuspenseQueries({
    queries: [
      useQueryDefine<DefineString>(databaseId),
      useQueryDefines<DefineString>('string'),
    ],
    combine: (results) => {
      return {
        data: results[0].data,
        stringList: removeDuplicateDefines(results[1].data),
      }
    },
  })
