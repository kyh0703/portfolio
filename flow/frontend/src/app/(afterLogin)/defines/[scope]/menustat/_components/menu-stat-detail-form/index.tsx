...
const schema = Yup.object().shape({
  id: Yup.string()
    ...
    .test(
      'max-bytes',
      '최대 20byte까지 입력 가능합니다.',
      (value) => new TextEncoder().encode(value).length <= 20,
    ),
})

export default function MenuStatDetailForm({
  scope,
  databaseId,
}: {
  scope: DefineScope
  ...
  const updateMutation = useUpdateDefine<DefineMenuStat>({
    onSuccess: () => {
  ...
  const { data } = useSuspenseQuery(useQueryDefine<DefineMenuStat>(databaseId))
